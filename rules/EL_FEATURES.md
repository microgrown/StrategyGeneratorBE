...
| `date` | **ASSUMED** | EL `Date` is the bar's date; the engine derives it from the bar's **close** timestamp. On an overnight session those are not the same day — @HO's session opens at 18:00, so we call the bars closing 19:00–23:00 on 2015-06-30 *June* where TradeStation may call them *July*. `MonthlyProfitTarget` pivots on that boundary and half the BAS-2 family carries it | `EL_Date_Probe.txt` — **written, not yet run** |
