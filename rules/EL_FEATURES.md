...
| `date[n]` `time[n]` | ACCEPTED | Series history of the Date/Time data words follows ordinary series indexing — the same bar store `close[n]` reads, nothing to compute. Needed by EEC X4 (`year(date[1])`, `month(date[1])`), E20 (`date<>date[1]`), E40 (`time[1]`) | — |
...
| `year` | VERIFIED | `Year(Date)` returns the calendar year **minus 1900** (2015 prints 115) of the bar's wall-clock close date — measured as the `y` column of the Date probe, 169/169 bars incl. evening and midnight bars. EEC X4 only tests `year(date) <> year(date[1])`, where the offset cancels | `EL_Date_Probe.txt` + `el_date_output.txt` |
...
| `highd` `lowd` `opend` `closed` | VERIFIED | **The day is the wall-clock CALENDAR date, not the session** — same boundary as `date <> date[1]`, so one slicing serves E20 and the xxxD family. On @HO the evening bars 1900–2300 form their own wall-clock day slice (Sunday is a 5-bar day; Monday's `HighD(1)` reads it, not Friday). `xxxD(0)` is the running value INCLUSIVE of the current bar; `OpenD(0)` is the date's first bar open. Warm-up: **−1 sentinel** while fewer than N complete days exist. Daily bars degrade exactly to series indexing (`HighD(n)` == `High[n]`, 58/58). Calendar model reproduced 412/412 intraday bars incl. MLK short day and weekends |
| `percentile` | VERIFIED | **Linear interpolation, inclusive endpoints (Excel PERCENTILE.INC)**: sort the window (bars 0..N−1, current bar INCLUDED), rank = pct·(N−1), interpolate — {1,2,3,4} at .25/.5/.75 → 1.75/2.5/3.25; interpolates across duplicates ({2,2,4,4} median = 3); sorts correctly on unordered windows. pct is a 0..1 FRACTION; out-of-range (25) returns a **−1 sentinel**, not an error. Window over a variable reads zero-filled warm-up history |
| `positionprofit` `totaltrades` | VERIFIED | `PositionProfit(1)` = the MOST RECENTLY CLOSED position, **net of the full round turn** (read exactly 20 below gross at commission 7 + slippage 3 per side); `(2)` = one older; `(0)` = the open position; `(1)` does not slide while a position is open. `TotalTrades` ticks on the exit's FILL bar (already incremented at that bar's close); a reversal's close counts once |
| `bollingerband` | VERIFIED | `Average(price, N) + NumDevs × StdDev` with the **population** deviation (sample refuted on 9,841/9,841 bars), fresh per bar, window incl. current, negative NumDevs = lower band. Summation is **newest-first** (`[0]` → `[N−1]`, the EL `Summation` order): the oldest-first twin drifts ≤2.8e-13 in exact arithmetic while the offline newest-first fit is at print noise (≤5.1e-13). Probe twin now patched newest-first for an optional bit-exact confirmation re-run |
| `moneyflow` | VERIFIED | `100 × pos / (pos + neg)` over 14 current-inclusive bars, money flow = `(H+L+C)/3 × Ticks` — **total ticks, not the up-tick `Volume` series**; a bar goes to pos when TP > TP[1] and to neg OTHERWISE (ties and downs share the else branch); fresh sums; **NO clamp**: with negative back-adjusted prices the ratio leaves 0..100 (598.8 measured on @OJ Aug 2008) — the mirror must not clamp either. Fitted 0/9,821 at 4.3e-10 |
| `stochastic` | VERIFIED | SmoothingType 1: `oFastK = 100·(C − LL(SLength)) / (HH − LL)` fresh (5.1e-13); **`oFastD` = `oSlowK` = `100·Sum(C − LL, Smoothing1) / Sum(HH − LL, Smoothing1)` — a RATIO OF SUMS, not an average of FastK** (that model refuted at maxd 24); `oSlowD` = simple Smoothing2-mean of SlowK (6.9e-13 in steady state). **Warm-up is SEED-FILLED, not zero-filled**: the num/den/SlowK series exist only from the study's first calculated bar, and a short sum pads its missing terms with the FIRST bar's value (SlowD's mean pads with the first SlowK) — measured exactly, 0/61 warm-up bars, by joining run 2's early bars with run 1's pre-history. Two chart starts differ only on the finite warm-up windows (43 bars) then agree exactly — no seed memory beyond that |
| `cci` | VERIFIED | Textbook `(TP − SMA(TP,N)) / (0.015 × mean abs deviation)`, TP = (H+L+C)/3, fresh, window incl. current — both summation orders fit within print noise (≤1.4e-11 on |CCI| ~100); **newest-first adopted** (EL `Summation` convention, and the oldest-first in-EL twin shows nonzero exact-arithmetic drift). Probe twin patched newest-first for an optional bit-exact confirmation re-run |
| `countif` | VERIFIED | Counts over bars 0..N−1 **including the current bar**; the condition is a true SERIES — its own offsets shift per counted bar (`A > A[1]` at position i tests `A[i] > A[i+1]`; the frozen-offset model refuted); warm-up counts over zero-filled variable history, bars before the chart contribute false |
| `ultimateosc` `ultimateoscillator` | VERIFIED | **`UltimateOscillator(7,14,28)` returns a FRACTION 0..1, not the ×100 textbook value**: UO = (4·A7 + 2·A14 + A28)/7 with Ax = Sum(C − TrueLow, x)/Sum(TrueRange, x), fresh, current-inclusive — 0/9,801 bars at 5e-13. B1's extremes/thresholds must be written on the fraction scale (the absolute 2.22e-12 tolerance is 100× more significant there). The book's `UltimateOsc` is the TS indicator's plot variable, not a function |
| `maxlist` `minlist` | ACCEPTED | **Scope: selection among tick-grid PRICE operands only** — differences are 0 or >= 1 tick, ~1e9-1e11x the tolerance, so which comparison they carry cannot matter (EEC E18's `minlist(l,l[1])` and E20's range maxlist qualify). Tolerance on COMPUTED operands (dollar amounts vs ATR-dollars) is still unmeasured and keeps X2 methods 3/4 blocked. Original concern: **Not EL source**, so whether they carry the relational tolerance is unestablished. Inert in the corpus today only because `AtrProfitTarget`'s stand-in loop compares tick-grid prices that differ by zero or by ≥1 tick — 1e9–1e11× the tolerance | `el_compare.h` says so explicitly |
...
| Feature | Status | What is known | Evidence |
|---|---|---|---|
| `/` on a zero denominator | VERIFIED | **Raises a runtime divide-by-zero error** (measured: the probe halted on its first zero-denominator bar). EEC E39's exact composite is therefore unsafe on any chart containing a zero-range bar; the practical mirror carries E38's +.000001 epsilon as a documented divergence — whether to also keep an exact-but-crashing variant is Brian's call |
...
| `crosses above` `crosses below` `crosses over` `crosses under` | VERIFIED | **A state machine, not a two-bar test.** Fires when the toleranced comparison goes above/below AND the most recent NON-equal state (under `el_eq`) was on the other side — an equality run is bridged, so above → equal → above does **not** re-fire (the `el_gt(A,B) && el_le(A[1],B[1])` mirror is REFUTED: it fires there). Tolerance (2.22e-12, absolute) applies on both sides: +1e-13 counts as equal, the cross fires on leaving the run; +1e-11 counts as above. `over`/`under` are exact synonyms (identical flags on 100/100 bars). No fire on the first bar (no prior side). Mirror needs a persistent last-side state per crossing site |
